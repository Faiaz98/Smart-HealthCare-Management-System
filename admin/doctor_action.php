...
require '../../shms/class/phpmailer/vendor/phpmailer/phpmailer/src/Exception.php';
...
if(isset($_POST["action"]))
{
	...
	if($_POST["action"] == 'change_status')
	{
		...
		$object->query = "SELECT patient_email_address from patient_table WHERE patient_id in(SELECT patient_id from appointment_table where doctor_id = '".$_POST["id"]."')";
		$object->execute();
		$result = $object->get_result();
		foreach($result as $row)
		{
			//$row['patient_email_address'];
			 ...
			 $mail->AddAddress($row['patient_email_address']);
			 $mail->IsHTML(true);
			 $mail->Subject = 'Change in status';

			 $message_body = '
			 <p>The doctor you have booked an appointment with has a change in status. Please visit the site for more information.</p>
			 <p>Sincerely,</p>
